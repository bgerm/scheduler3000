...
import positionEvents from '../../src/components/Scheduler/AllDay/positionEvents';
import { pick } from 'lodash';

describe('positionEvents', function () {
  it('Should position events in their right place', function () {
    const normalizedEvents = Immutable.fromJS({
      events: {
        '1': {
          id: '1',
          title: 'event1',
    ...
    const weekEvents = normalizedEvents.get('weekEvents').toJS();

    const startDate = moment.tz('2016-01-01 00:00:00', 'UTC');
    const endDate = moment.tz('2016-01-07 23:59:59', 'UTC');

    const positioned = positionEvents(weekEvents, normalizedEvents.get('events'), startDate, endDate);
  ...
  it('Should collapse collapse events with more event (test 1)', function () {
    ...
    const positioned = positionEvents(weekEvents, normalizedEvents.get('events'), startDate, endDate, 1);
  ...
  it('Should collapse collapse events with more event (test 2)', function () {
    ...
    const positioned = positionEvents(weekEvents, normalizedEvents.get('events'), startDate, endDate, 2);
